...
LAUNCH_JSON=$(wget -qO- "https://api.github.com/repos/BandiFee/SwitchVN-Switchdeck/contents/files/steam/launch-steam.sh?ref=main")
UPDATE_JSON=$(wget -qO- "https://api.github.com/repos/BandiFee/SwitchVN-Switchdeck/contents/files/steam/update-switchdeck.sh?ref=main")
...
if [ "$LATEST_LAUNCH_SHA" != "$(cat "$LAUNCH_SHA_FILE" 2>/dev/null)" ] || [ ! -f "$STEAMROOT/launch-steam.sh" ]; then
    wget -q --header="Accept: application/vnd.github.v3.raw" -O "$STEAMROOT/launch-steam.sh" "https://api.github.com/repos/BandiFee/SwitchVN-Switchdeck/contents/files/steam/launch-steam.sh?ref=main"
...
if [ "$LATEST_UPDATE_SHA" != "$(cat "$UPDATE_SHA_FILE" 2>/dev/null)" ] || [ ! -f "$STEAMROOT/update-switchdeck.sh" ]; then
    wget -q --header="Accept: application/vnd.github.v3.raw" -O "$STEAMROOT/update-switchdeck.sh" "https://api.github.com/repos/BandiFee/SwitchVN-Switchdeck/contents/files/steam/update-switchdeck.sh?ref=main"
